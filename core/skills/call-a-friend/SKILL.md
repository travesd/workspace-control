---
name: call-a-friend
description: Invoke a fresh agent perspective for bounded reviews, second opinions, or explicitly approved delegated implementation from a detailed plan.
---

# Call A Friend

Use this skill when the user asks to "call a friend", asks another agent or
provider to review work, wants a fresh cross-provider second opinion, or
explicitly approves offloading implementation from a detailed plan.
...
1. Define the friend task: mode, goal, repo/path, branch or workspace context,
   files in scope, out-of-scope areas, and expected output shape.
...
8. Record the call in task notes or handoff state: provider, model/effort if
   known, command shape, time, scope, outcome, and any limitations.

## Provider Commands

When an actual CLI invocation is needed, read the local provider adapter or
provider command reference. Keep provider-specific command syntax out of this
shared skill body.
...
branches/workspaces, clean task artifacts, or activate live changes unless the
user explicitly approved that exact action.

## Review Prompt Checklist

- What changed or is being proposed.
- Files, branch, task directory, workspace, or diff to inspect.
...
- Constraints: read-only, no secrets, no external systems unless approved, no
  activation, no push/PR/merge.

## Guardrails

- Do not interrupt active sessions to call a friend.
